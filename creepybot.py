...
class CreepyBot(sc2.BotAI):
    ...
    def select_build_worker(self, pos, force=False, excludeTags=[]):
        ...
        for worker in workers.prefer_close_to(pos).prefer_idle:
            if not worker.orders or len(worker.orders) == 1 and worker.orders[0].ability.id in [AbilityId.MOVE, AbilityId.HARVEST_GATHER, AbilityId.HARVEST_RETURN] and worker.tag not in excludeTags:
                return worker
    ...
    async def on_step(self, iteration):
        ...
        self.currentDroneCountIncludingPending = self.units(DRONE).amount + self.already_pending(DRONE) + self.units(EXTRACTOR).ready.filter(lambda x:x.vespene_contents > 0).amount
        ...
        if iteration % self.buildStuffInverval == 0:
            ...
            larvaPerSecond = self.townhalls.ready.amount * self.larvaPerHatch + self.units(QUEEN).ready.amount * self.larvaPerInject
            ...
            if self.supply_left + self.already_pending(OVERLORD) * 8 < 2 * larvaPer18Seconds and self.supply_cap + self.already_pending(OVERLORD) * 8 < 200 and self.supply_used >= 13 or self.supply_cap < 14:
            ...
            if self.townhalls.amount >= 2 and self.units(EXTRACTOR).amount + self.already_pending(EXTRACTOR) < 1 and (self.getLingSpeed or (self.currentDroneCountIncludingPending > 25 and self.units(SPAWNINGPOOL).exists)):
            ...
            if self.currentDroneCountIncludingPending > 17 and not self.units(SPAWNINGPOOL).exists and self.already_pending(SPAWNINGPOOL) < 1 and self.townhalls.amount >= 2:
                self.workerProductionEnabled.append(False)
            ...
            if ((self.units(CREEPTUMOR) | self.units(CREEPTUMORBURROWED) | self.units(CREEPTUMORQUEEN)).amount >= 2 or not self.enableCreepSpread) and self.enableInjects:
                if self.units(GREATERSPIRE).exists:
            ...
            if (self.currentDroneCountIncludingPending >= 40 or self.opponentInfo["armySupplyScouted"] > 5) and self.units(ROACHWARREN).amount + self.already_pending(ROACHWARREN) < 1 and self.townhalls.exists and self.enableMakingRoaches:
            ...
            if self.currentDroneCountIncludingPending >= 37 and self.units(EVOLUTIONCHAMBER).amount + self.already_pending(EVOLUTIONCHAMBER) < 2 and self.townhalls.amount > 2:
                # self.workerProductionEnabled.append(False) # priotize?
                if self.can_afford(EVOLUTIONCHAMBER):
                    pos = await self.find_placement(EVOLUTIONCHAMBER, townhallLocationFurthestFromOpponent, min_distance=6)
            ...
            if self.currentDroneCountIncludingPending >= 66 and self.units(EXTRACTOR).filter(lambda x:x.vespene_contents > 0).amount + self.already_pending(EXTRACTOR) < 9 and self.already_pending(EXTRACTOR) < 4 and self.townhalls.amount > 2:
                self.workerProductionEnabled.append(False)
            ...
            if self.getTimeInSeconds() > 4.5 * 60 and self.currentDroneCountIncludingPending > 50:
                if self.already_pending(LAIR) < 1 and self.units(HATCHERY).ready.idle.exists and self.units(SPAWNINGPOOL).ready.exists and not (self.units(LAIR) | self.units(HIVE)).exists and self.vespene >= 100:
                ...
                if (self.supply_used > 130 or not self.enableMakingRoaches) and self.units(LAIR).ready.amount > 0 and self.units(INFESTATIONPIT).amount + self.already_pending(INFESTATIONPIT) < 1 and self.townhalls.amount > 3:
                    self.armyProductionEnabled.append(False)
                    self.workerProductionEnabled.append(False)
                    pos = await self.find_placement(INFESTATIONPIT, townhallLocationFurthestFromOpponent, min_distance=6)
                ...
                # morph to hive
                if self.supply_used > 150 and self.units(LAIR).ready.idle.exists and self.units(INFESTATIONPIT).ready.exists and not self.already_pending(HIVE) and not self.units(HIVE).exists and self.townhalls.amount > 3:
                    self.armyProductionEnabled.append(False)
                    self.workerProductionEnabled.append(False)
                ...
                if self.supply_used > 80 and (self.already_pending(HIVE) or self.units(HIVE).exists) and self.units(GREATERSPIRE).amount + self.already_pending(GREATERSPIRE) < 1 and self.enableLateGame:
                    ...
                    elif not self.units(GREATERSPIRE).exists and not self.units(SPIRE).exists and self.already_pending(SPIRE) < 1:
                        self.armyProductionEnabled.append(False)
                        self.workerProductionEnabled.append(False)
                        if self.can_afford(SPIRE):
                            pos = await self.find_placement(SPIRE, townhallLocationFurthestFromOpponent, min_distance=6, minDistanceToResources=2)
            ...
            if self.units(ROACHWARREN).ready.amount > 0 and (self.units(LAIR).ready.amount > 0 or self.units(HIVE).amount > 0) and self.enableMakingRoaches:
                if self.units(ROACHWARREN).ready.idle.amount > 0:
                    ...
                    elif self.getRoachBurrowAndBurrow and AbilityId.RESEARCH_TUNNELINGCLAWS in abilities and self.can_afford(AbilityId.RESEARCH_TUNNELINGCLAWS):
                ...
                abilities = await self.get_available_abilities(rw)
                if self.getRoachBurrowAndBurrow and AbilityId.RESEARCH_TUNNELINGCLAWS not in abilities and self.can_afford(AbilityId.RESEARCH_TUNNELINGCLAWS) and self.can_afford(AbilityId.RESEARCH_BURROW):
                    if self.iteration % self.creepSpreadInterval == 0:
                        ...
                        if drone is not None:
                            ...
                            if AbilityId.BURROWDOWN_DRONE not in droneAbilities and self.can_afford(AbilityId.RESEARCH_BURROW):
            ...
            if self.minerals > 1000 and self.vespene > 1000 and self.can_afford(AbilityId.RESEARCH_PNEUMATIZEDCARAPACE) and self.units(HATCHERY).idle.exists:
            ...
            if self.enableLateGame and self.units(GREATERSPIRE).ready.idle.exists: # and self.can_afford(RESEARCH_ZERGFLYERATTACKLEVEL3):
            ...
            inNeedOfExpansion = self.getTimeInSeconds() > 10*60 and sum([self.state.mineral_field.closer_than(10, x).amount for x in self.townhalls]) < 32 or self.currentDroneCountIncludingPending + 16 > (self.townhalls.amount) * 16 + self.units(EXTRACTOR).ready.filter(lambda x:x.vespene_contents > 0).amount * 3

            if self.townhalls.amount > 1:
                self.haltRedistributeWorkers = False
            # send worker to the first expansion to build it asap
            if self.townhalls.amount < 2 and (self.currentDroneCountIncludingPending > 17 or (self.can_afford(HATCHERY) and self.units(DRONE).amount > 5)) and self.nextExpansionInfo == {} and self.already_pending(HATCHERY) < 1:
                location = await self.get_next_expansion()
                w = self.select_build_worker(location, excludeTags=self.reservedWorkers)
                loc = await self.find_placement(HATCHERY, near=location, random_alternative=False, placement_step=1, minDistanceToResources=5, max_distance=20)
                if loc is not None:
                    location = loc
                ...
                if self.can_afford(HATCHERY):
                    ...
                    if w is not None:
                        ...
                        print(self.getTimeInSeconds(), "building first expansion with worker", w.tag, self.nextExpansionInfo["workerTag"])
            ...
            elif (self.currentDroneCountIncludingPending > self.townhalls.amount * 16 or (self.can_afford(HATCHERY) and self.townhalls.amount < 3 or self.minerals > 2000)) and self.townhalls.amount > 1 and inNeedOfExpansion and self.already_pending(HATCHERY) < 1:
                self.workerProductionEnabled.append(False)
                if self.can_afford(HATCHERY):
                    location = await self.get_next_expansion()
        ...
        if self.enableCreepSpread and iteration % self.creepSpreadInterval == 0 and (self.getTimeInSeconds() > 3 * 60 or self.opponentInfo["armySupplyScouted"] < 5 or (self.units(CREEPTUMOR) | self.units(CREEPTUMORBURROWED) | self.units(CREEPTUMORQUEEN)).amount < 2):
        ...
        if iteration % self.buildStuffInverval == 0:
            ...
            if self.units(ZERGLING).amount + self.already_pending(ZERGLING) + len(self.scoutingUnits) < self.opponentInfo["scoutingUnitsNeeded"] and self.supply_left > 1 and self.units(SPAWNINGPOOL).ready.exists and self.units(LARVA).exists and self.supply_used < 198:
            ...
            for ling in self.units(ZERGLING).filter(lambda x:x.health >= 20):
                if len(self.scoutingUnits) < self.opponentInfo["scoutingUnitsNeeded"] and ling.tag not in self.scoutingUnits:
            ...
            makeDefensiveRoaches = self.opponentInfo["armySupplyScoutedClose"] // 1 > self.units.not_structure.filter(lambda x:x.type_id not in [DRONE, LARVA, OVERLORD, ZERGLING]).amount + self.already_pending(ROACH) + self.already_pending(QUEEN) + self.units(SPINECRAWLER).not_ready.amount and self.getTimeInSeconds() < 6*60 and self.vespene > 25
            ...
            if self.units(ROACHWARREN).ready.exists and \
                ...
                ((self.minerals > 400 and self.vespene > 400 or not self.units(EVOLUTIONCHAMBER).idle.exists and self.currentDroneCountIncludingPending >= self.droneLimit) or makeDefensiveRoaches) and self.supply_used < 197:
                if self.supply_left > 1 and self.can_afford(ROACH):
                    roachesAffordable = min((196 - self.supply_used)//2, self.supply_left // 2, self.minerals // 75, self.vespene // 25, self.units(LARVA).amount)
            ...
            if all(self.armyProductionEnabled) and (self.units(LAIR) | self.units(HIVE)).exists and (self.units(OVERSEER) | self.units(OVERLORDCOCOON)).amount < 3 and hasattr(self, "overlordsSendlocation") and (self.minerals > 500 and self.vespene > 500 or self.supply_used > 150):
            ...
            makeBroodlords = corruptorsTotal / self.corruptorRatioFactor > broodlordsTotal / self.broodlordRatioFactor and self.units(CORRUPTOR).amount > 1
            makeVipers = corruptorsTotal / self.corruptorRatioFactor > vipersTotal / self.viperRatioFactor

            if all(self.armyProductionEnabled) and self.units(GREATERSPIRE).exists and self.can_afford(CORRUPTOR) and self.supply_left > 1 and makeCorruptors: # leave a bit supply for viper and BL morphs
                corruptorAffordable = min(self.supply_left // 2, self.minerals // 150, self.vespene // 100, self.units(LARVA).amount)
                for count, larva in enumerate(self.units(LARVA)):
            ...
            elif all(self.armyProductionEnabled) and self.units(GREATERSPIRE).exists and self.units(CORRUPTOR).idle.exists and self.supply_left > 1 and makeBroodlords:
            ...
            elif all(self.armyProductionEnabled) and self.units(GREATERSPIRE).exists and self.can_afford(VIPER) and self.supply_left > 5 and makeVipers: # leave a bit supply for viper and BL morphs
        ...
        if iteration % self.workerTransferInterval == 0:
            ...
            if not hasattr(self, "overlordsSendlocation"):
                self.overlordsSendlocation = OrderedDict({base.position.to2: set() for base in self.exactExpansionLocations})
                self.overlordsSendlocation = OrderedDict(sorted(self.overlordsSendlocation.items(), key=lambda x: x[0].closest(self.enemy_start_locations).distance_to(x[0])))
            ...
            myBasesAndEnemyBases = [x.position.to2 for x in self.townhalls | self.known_enemy_structures.filter(lambda x:x.type_id in [HATCHERY, LAIR, HIVE, COMMANDCENTER, PLANETARYFORTRESS, ORBITALCOMMAND, NEXUS] and x.build_progress > 0.5)]
            if len(myBasesAndEnemyBases) > 0:
                for key, value in self.overlordsSendlocation.items():
                    ...
                    if unassignedOverlords.exists and len(value) == 0 and closestBaseNearTargetLocation.distance_to(key) > 15:
                        ov = unassignedOverlords.pop()
                        value.add(ov.tag)
                        await self.do(ov.move(key))
                    # move overlord back and unassign if base was taken by someone
                    elif len(value) > 0 and closestBaseNearTargetLocation.distance_to(key) <= 15 and self.townhalls.exists:
            ...
            if iteration % (self.workerTransferInterval * 10) == 0:
                ...
                if self.units(OVERLORD).exists:
                    for ov in self.units(OVERLORD):
                        ...
                        if AbilityId.BEHAVIOR_GENERATECREEPON in abilities and self.can_afford(BEHAVIOR_GENERATECREEPON):
            ...
            for ch in self.units.filter(lambda x:x.type_id in [CHANGELING, CHANGELINGZEALOT, CHANGELINGMARINESHIELD, CHANGELINGMARINE, CHANGELINGZERGLINGWINGS, CHANGELINGZERGLING]).idle:
                if self.known_enemy_structures.exists:
                    await self.do(ch.move(self.known_enemy_structures.random.position))

            ignoreUnits = [OVERSEER, OBSERVERSIEGEMODE, OVERSEERSIEGEMODE, PHOTONCANNON, MISSILETURRET, RAVEN, SPORECRAWLER]

            # move overlords away from enemies
            for ov in self.units(OVERLORD).filter(lambda x:x.tag not in assignedOverlordTags):
                if ov.health < ov.health_max and (ov.is_idle or len(ov.orders) == 1 and ov.orders[0].ability.id in [AbilityId.MOVE]) and self.townhalls.exists:
            ...
            if iteration % self.workerTransferInterval * 2 == 0 and self.getTimeInSeconds() > 6*60:
                ...
                for drone in self.units(DRONE):
                    if self.known_enemy_units.not_structure.exists:
                        nearbyEnemies = self.known_enemy_units.not_structure.closer_than(15, drone.position).filter(lambda x:x.type_id not in ignoreUnits)
        ...
        if iteration % self.microInterval == 0 and hasattr(self, "scoutingUnits"):
            ...
            for lingTag in list(self.scoutingUnits)[:]:
                ...
                scoutUnitInDanger = (self.known_enemy_units.not_structure.closer_than(10, ling).filter(lambda x: self.getSpecificUnitInfo(x)[0]["dps"] > 5) | self.known_enemy_structures.filter(lambda x: x.type_id in [BUNKER, SPINECRAWLER, PHOTONCANNON])).exists or ling.health < 20
                if scoutUnitInDanger and self.townhalls.exists:
                    if ling.health < 20:
                        self.scoutingUnits.discard(lingTag)
                    if len(ling.orders) == 1 and ling.orders[0].ability.id in [AbilityId.ATTACK]: # here: attack-move means actively scouting a target
        ...
        ignoreScoutingBuildings = [SUPPLYDEPOT, SUPPLYDEPOTLOWERED, BARRACKS, BUNKER, SPINECRAWLER, SPORECRAWLER, NYDUSNETWORK, NYDUSCANAL, PYLON, PHOTONCANNON, GATEWAY]
        if self.opponentInfo["spawnLocation"] is None and len(self.enemy_start_locations) > 0:
            if self.known_enemy_units.structure.exists:
                enemyUnitsFiltered = self.known_enemy_units.structure.filter(lambda x:x.type_id not in ignoreScoutingBuildings)
                if enemyUnitsFiltered.exists:
                    self.opponentInfo["spawnLocation"] = enemyUnitsFiltered.random.position.closest(self.enemy_start_locations)
        ...
        visibleEnemyUnits = self.known_enemy_units.not_structure.filter(lambda x:x.type_id not in [DRONE, SCV, PROBE, LARVA, EGG])
        ...
        self.opponentInfo["armySupplyScouted"] = sum(x["supply"] for x in self.opponentInfo["armyTagsScouted"] if x["scoutTime"] > tempTime)
        self.opponentInfo["armySupplyScoutedClose"] = sum(x["supply"] for x in self.opponentInfo["armyTagsScouted"] if x["scoutTime"] > tempTime and x["distanceToBase"] < 60)
        self.opponentInfo["armySupplyVisible"] = sum(self.getUnitInfo(x) or 0 for x in visibleEnemyUnits)

        # get opponent expansions
        if iteration % 20 == 0:
            enemyTownhalls = self.known_enemy_units.structure.filter(lambda x:x.type_id in [HATCHERY, LAIR, HIVE, COMMANDCENTER, PLANETARYFORTRESS, ORBITALCOMMAND, NEXUS])
            for th in enemyTownhalls:
                if len(self.opponentInfo["expansions"]) > 0 and th.position.closest(self.opponentInfo["expansions"]).distance_to(th.position.to2) < 20:
                    continue
        ...
        if iteration % self.microInterval == 0:
            ...
            for roach in self.units(ROACHBURROWED):
                if 9/10 <= roach.health / roach.health_max <= 2 and roach.is_burrowed:
                    ...
                    nearbyDetection = self.known_enemy_units.filter(lambda x:x.type_id in [OVERSEER, OBSERVERSIEGEMODE, OVERSEERSIEGEMODE, PHOTONCANNON, MISSILETURRET, RAVEN, SPORECRAWLER])
        ...
        if iteration % self.microInterval == 0:
            ...
            if self.units(QUEEN).exists:
                ...
                transfuseTargets = self.units.filter(lambda x: x.tag not in targetsBeingTransfusedTags and x.type_id in [QUEEN, BROODLORD, CORRUPTOR, SPINECRAWLER, OVERSEER, ROACH] and (x.health_max - x.health >= 125 or x.health / x.health_max < 1/4))
                transfuseTargetsBuildings = self.units.structure.filter(lambda x: x.health_max - x.health > 125 and x.type_id not in [SPINECRAWLER])
                ...
                for q in queensWith50Energy:
                    ...
                    if transfuseTarget.distance_to(q) < 7: # replace with general queen transfuse range
                        ...
                        if AbilityId.TRANSFUSION_TRANSFUSION in abilities and self.can_afford(TRANSFUSION_TRANSFUSION):
                            self.transfuseProcess[q.tag] = {"target": transfuseTarget.tag, "expiryTime": self.getTimeInSeconds() + 1}
                ...
                for q in queensWith190Energy:
                    ...
                    if transfuseTarget.distance_to(q) < 7: # replace with general queen transfuse range
                        ...
                        if AbilityId.TRANSFUSION_TRANSFUSION in abilities and self.can_afford(TRANSFUSION_TRANSFUSION):
                            self.transfuseProcess[q.tag] = {"target": transfuseTarget.tag, "expiryTime": self.getTimeInSeconds() + 1}
                            targetsBeingTransfusedTags.add(transfuseTarget.tag)
                            transfuseTargetsBuildings = transfuseTargetsBuildings.filter(lambda x:x.tag not in targetsBeingTransfusedTags)
                            await self.do(q(TRANSFUSION_TRANSFUSION, transfuseTarget))

            for vp in self.units(VIPER).filter(lambda x:x.is_idle or len(x.orders) == 1 and x.orders[0].ability.id in [AbilityId.MOVE, AbilityId.SCAN_MOVE, AbilityId.ATTACK]):
                ...
                if vp.energy < 125:
                    ...
                    # para bomb, priotize!
                    if vp.energy >= 125:
                        enemyAirUnits = self.known_enemy_units.not_structure.filter(lambda x:x.type_id in [MUTALISK, VIKINGFIGHTER, CORRUPTOR, BANSHEE, RAVEN, VOIDRAY, PHOENIX, ORACLE, VIPER] and not x.has_buff(PARASITICBOMB) and x.is_flying).closer_than(13, vp.position)
                    ...
                    if vp.energy >= 100:
                        ...
                        blindCloudTargets = self.known_enemy_units.not_structure.filter(lambda x:x.type_id in targets and not x.has_buff(BLINDINGCLOUD)).closer_than(12, vp.position)
                        if not blindCloudTargets.exists:
                            blindCloudTargets = self.known_enemy_units.not_structure.filter(lambda x:x.type_id.value in targets2 and not x.has_buff(BLINDINGCLOUD)).closer_than(12, vp.position)
                    ...
                    # abduct
                    if vp.energy >= 75:
                        targets = [SIEGETANKSIEGED, THOR, LURKER, LURKERMPBURROWED, ARCHON, MOTHERSHIP, CARRIER, TEMPEST, LIBERATOR]
                        targets2 = [QUEEN, STALKER, SIEGETANK, MARAUDER, CYCLONE]
                        abductTargets = self.known_enemy_units.not_structure.filter(lambda x:x.type_id in targets).closer_than(11, vp.position)
                        if not abductTargets.exists:
                            abductTargets = self.known_enemy_units.not_structure.filter(lambda x:x.type_id in targets2).closer_than(11, vp.position)
            ...
            if iteration % self.microInterval*10 == 0:
                if (not self.known_enemy_structures.exists):
                    ...
                    pointsOnPlayableMap = [Point2((x, y)) \
                        for x in range(self._game_info.playable_area[0]+stepSize, self._game_info.playable_area[0] + self._game_info.playable_area[2], stepSize) \
                        for y in range(self._game_info.playable_area[1]+stepSize, self._game_info.playable_area[1] + self._game_info.playable_area[3], stepSize)]
        ...
        unitsAssignedToGroups = set().union(*[group.getMyUnitTags() for group in [self.myDefendGroup, self.myAttackGroup, self.mySupportGroup] if group is not None])
        ...
        if iteration % self.microInterval*5 == 0:
            ...
            if self.getTimeInSeconds() < 5*60 and self.opponentInfo["armySupplyScoutedClose"] > 2:
                ...
                self.myDefendGroup.removeDefense(self.units(QUEEN).filter(lambda q: q.tag in self.queensAssignedHatcheries))
            ...
            if self.myAttackGroup is None:
                if all([
                    ...
                    not self.waitForRoachBurrowBeforeAttacking or (self.units(DRONE).exists and AbilityId.BURROWDOWN_DRONE in (await self.get_available_abilities(self.units(DRONE).random))),
                    ...
                    myArmy = self.units(ROACH) | self.units(ROACHBURROWED) | self.units(BROODLORD) | self.units(BROODLORDCOCOON) | self.units(EGG).filter(lambda x: x.orders[0].ability in [ROACH])
                    mySupportUnits = self.units(VIPER) | self.units(CORRUPTOR) | self.units(OVERSEER) | self.units(EGG).filter(lambda x: x.orders[0].ability in [CORRUPTOR, VIPER])
                    if hasattr(self, "queensAssignedHatcheries"):
                        mySupportUnits.extend(self.units(QUEEN).filter(lambda x: x.tag not in self.queensAssignedHatcheries))
                    ...
                    if self.supply_used >= 195:
                        ...
                        knownEnemyUnitsFiltered = self.known_enemy_units.filter(lambda x: x.type_id not in [OVERLORD, OVERSEER, OVERLORDTRANSPORT, OVERSEERSIEGEMODE])
                ...
                myArmy = self.units(ROACH) | self.units(ROACHBURROWED) | self.units(BROODLORD) | self.units(BROODLORDCOCOON)
                mySupportUnits = self.units(VIPER) | self.units(CORRUPTOR) | self.units(OVERSEER)
                knownEnemyUnitsFilteredInRange = self.known_enemy_units.filter(lambda x: x.type_id not in [OVERLORD, OVERSEER, OVERLORDTRANSPORT, OVERSEERSIEGEMODE] and myArmy.closer_than(25, x.position).exists)
                knownEnemyUnitsFilteredOutsideRange = self.known_enemy_units.filter(lambda x: x.type_id not in [OVERLORD, OVERSEER, OVERLORDTRANSPORT, OVERSEERSIEGEMODE] and not myArmy.closer_than(30, x.position).exists)
        ...
        if iteration % self.microInterval == 0:
            if self.myDefendGroup is not None:
                ...
                await self.myDefendGroup.update(self.units, self.known_enemy_units, self.enemy_start_locations, iteration)
            if self.myAttackGroup is not None:
                # contains roaches, burrow roaches, broodlords
                # this group will not retreat
                await self.myAttackGroup.update(self.units, self.known_enemy_units, self.enemy_start_locations, iteration)
            if self.mySupportGroup is not None:
                # contains queens, corruptors, vipers
                await self.mySupportGroup.update(self.units, self.known_enemy_units, self.enemy_start_locations, iteration)
        ...
        if iteration % self.buildStuffInverval == 0 and 120 < self.getTimeInSeconds() < 240 and self.opponentInfo["armySupplyScoutedClose"] > 2 and len(self.opponentInfo["expansions"]) < 2 and self.townhalls.ready.amount > 1 and self.units(SPINECRAWLER).amount < min(3, self.opponentInfo["armySupplyScoutedClose"] // 2) and self.already_pending(SPINECRAWLER) < 1:
            ...
            if self.can_afford(SPINECRAWLER) and self.workers.exists and self.townhalls.exists and furthestExpansion is not None:
        ...
        if iteration % self.buildStuffInverval == 0:
            airUnits = self.known_enemy_units.filter(lambda x:x.type_id in [ORACLE, BANSHEE, MUTALISK, CORRUPTOR, DARKSHRINE, STARGATE, PHOENIX, DARKTEMPLAR]) # STARPORTTECHLAB
            if airUnits.exists and self.already_pending(SPORECRAWLER) < 1:
                ...
                if self.can_afford(SPORECRAWLER):
                    for th in self.townhalls.ready:
                        ...
                        if self.units(SPORECRAWLER).exists and self.units(SPORECRAWLER).closest_to(th).distance_to(th) < 10:
                            continue
                        ...
                        if self.centerOfUnits(mfs).to2 != th.position.to2:
                            ...
                            loc = await self.find_placement(SPORECRAWLER, loc, placement_step=1, minDistanceToResources=0)
                            w = self.workers.closest_to(loc)
        ...
        if self.townhalls.exists and self.getTimeInSeconds() < 5*60:
            ...
            if (pylons.amount + probes.amount > 0 or cannons.amount > 0) and self.opponentInfo["armySupplyVisible"] < 3 and self.units(SPINECRAWLER).ready.amount < 1 and self.units(QUEEN).amount < 4:
                if not hasattr(self, "defendCannonRushProbes"):
                    self.defendCannonRushProbes = {}
                    self.defendCannonRushCannons = {}

                assignedDroneTagsSets = [x for x in self.defendCannonRushProbes.values()] + [x for x in self.defendCannonRushCannons.values()]
        ...
        # if pool is done, make queens
        if all(self.queenProductionEnabled) and self.units(SPAWNINGPOOL).ready.exists and self.units(QUEEN).amount + self.already_pending(QUEEN) < self.totalQueenLimit and (self.units(HATCHERY) | self.units(HIVE)).ready.idle.exists and iteration % self.buildStuffInverval == 0 and self.supply_left > 1 and self.supply_used < 197:
        ...
        if all(self.workerProductionEnabled) and iteration % self.buildStuffInverval == 0:
            if self.supply_left > 0 and self.supply_used < 198 and self.can_afford(DRONE) and self.units(LARVA).exists and self.currentDroneCountIncludingPending < self.droneLimit:
                dronesAffordable = min(198 - self.supply_used, self.droneLimit - self.units(DRONE).amount + self.already_pending(DRONE), self.supply_left // 1, self.minerals // 50, self.units(LARVA).amount)
