...
class MadBot(sc2.BotAI):
    def __init__(self):
        ...
        # self.model = keras.models.load_model("MadAI_13_08_2019")
    ...
    async def game_won(self):
        if (
            not self.won
            and len(self.known_enemy_units(NEXUS))
            ...
            + len(self.known_enemy_units(UnitTypeId.LAIR))
            + len(self.known_enemy_units(UnitTypeId.HIVE))
            == 0
        ):
            self.won = True
            self.train_data.append(
        ...
        if self.units(GATEWAY).ready.exists:
            for gw in self.units(GATEWAY).ready.noqueue:
                if self.can_afford(ZEALOT) and self.supply_used <= 22:
                    ...
                    and (
                        len(self.known_enemy_units(UnitTypeId.HATCHERY)) >= 1
                ...
                if self.units(CYBERNETICSCORE).ready.exists and not self.early_game_finished:
                    ...
                    for depotlow in range(len(self.known_enemy_units(UnitTypeId.SUPPLYDEPOTLOWERED))):
                        enemy_depotlow_pos.append(
                            self.known_enemy_units(UnitTypeId.SUPPLYDEPOTLOWERED)[depotlow].position
                        )
                    enemy_bunker_pos = []
                    ...
                    self.scout_data = [
                        len(self.known_enemy_units(NEXUS)),
                    ...
                    # if self.build_order == 0:
                        ...
                        # await self.chat_send(
                            # "(glhf) MadBot v1.9: 2-Base Colossus BO chosen! Certainties: 2-Base Colossus: "
                    ...
                    # elif self.build_order == 1:
                        # print("--- One-Base Defend into DT BO chosen ---")
                        # await self.chat_send(
                            # "(glhf) MadBot v1.9: Rush-Defend into DT BO chosen! Certainties: 2-Base Colossus: "
                            # + str(round(prediction[0][0] * 100, 2))
                            # + "%; "
                            # + "1-Base DTs: "
                            # + str(round(prediction[0][1] * 100, 2))
                    ...
                    # elif self.build_order == 2:
                        # print("--- 4-Gate Proxy BO chosen ---")
                        # await self.chat_send(
                            # "(glhf) MadBot v1.9: 4-Gate Proxy BO chosen! Certainties: 2-Base Colossus: "
                            # + str(round(prediction[0][0] * 100, 2))
                            # + "%; "
                            # + "1-Base DTs: "
                            # + str(round(prediction[0][1] * 100, 2))
                    ...
                    # elif self.build_order == 3:
                        ...
                        # await self.chat_send(
                            # "(glhf) MadBot v1.9: 2-Base Immortal BO chosen! Certainties: 2-Base Colossus: "
                            # + str(round(prediction[0][0] * 100, 2))
                            # + "%; "
                            # + "1-Base DTs: "
                            # + str(round(prediction[0][1] * 100, 2))
                    ...
                    # elif self.build_order == 4:
                        # print("--- One-Base Defend into Voidrays BO chosen ---")
                        # await self.chat_send(
                            # "(glhf) MadBot v1.9: Rush-Defend into Voidrays BO chosen! Certainties: 2-Base Colossus: "
                            # + str(round(prediction[0][0] * 100, 2))
                            # + "%; "
                            # + "1-Base DTs: "
                            # + str(round(prediction[0][1] * 100, 2))
                    ...
                    # else:
                        # self.build_order = random.randrange(0, 5)
                        # await self.chat_send("(glhf) MadBot v1.9: Neural Network broke, choosing random Build Order")
                    # if str(self.enemy_race) == 'Race.Protoss':
                    #     self.build_order = 4
                    #     await self.chat_send(
                    #         "Well, screw it! Lets try something different!"
                    #     )
                    # if str(self.enemy_race) == 'Race.Terran':
                    #     self.build_order = 2
                    #     await self.chat_send(
                    #         "Well, screw it! Lets try something different!"
                    #     )
    async def defend_early_rush(self):
        ...
        if (
            len(self.units(ZEALOT)) + len(self.units(STALKER)) + len(self.units(ADEPT)) < 2 and self.known_enemy_units
            ...
            # if self.units(NEXUS).exists and self.defend_early:
            #     for pr in self.units(PROBE).ready.further_than(15, self.units(NEXUS).first.position):
        ...
        if self.first_pylon_built and self.units(PYLON).exists and self.scout == []:
            ...
            if self.time > self.do_something_scout:
                ...
                for scout in self.scout:
                    ...
                    move_to5 = self.enemy_start_locations[0].towards(
                        self.game_info.map_center, random.randrange(-10, -1)
                    )
    ...
    async def expand(self):
        if (
            self.units(NEXUS).exists
            and self.units(NEXUS).amount < self.MAX_EXE
    ...
    async def chrono_boost(self):
        if self.units(NEXUS).ready.exists:
            ...
            if not self.units(GATEWAY).ready.exists and not self.units(WARPGATE).ready.exists:
                ...
                self.units(WARPGATE).ready.exists
                and not self.units(ROBOTICSFACILITY).ready.exists
                and not self.units(STARGATE).ready.exists
                and not self.units(GATEWAY).ready.exists
            ):
                ...
                not self.units(ROBOTICSFACILITY).ready.exists
                and not self.units(STARGATE).ready.exists
    ...
    async def micro_units(self):
        ...
        for st in self.units(STALKER):
            ...
            if st.shield_percentage < 0.1:
                threats = self.known_enemy_units.not_structure.filter(
        ...
        for im in self.units(IMMORTAL):
            ...
            if im.shield_percentage < 0.1:
                threats = self.known_enemy_units.filter(
                    lambda unit: unit.type_id not in self.units_to_ignore
                ).closer_than(im.ground_range + im.radius, im.position)
                if threats.exists and im.position != threats.closest_to(im).position:
                    ...
                    if distance is None:
                        ...
                        self.combinedActions.append(im.move(im.position.towards(threats.closest_to(im).position, -2)))
        ...
        for cl in self.units(COLOSSUS):
            ...
            if cl.shield_percentage < 0.1:
                threats = self.known_enemy_units.not_structure.filter(
                    lambda unit: unit.type_id not in self.units_to_ignore
                ).closer_than(cl.ground_range + cl.radius, cl.position)
                if threats.exists and cl.position != threats.closest_to(cl).position:
                    ...
                    if distance is None:
                        ...
                        self.combinedActions.append(cl.move(cl.position.towards(threats.closest_to(cl).position, -2)))
            ...
            if se.shield_percentage < 0.1:
                if threats.exists and se.position != threats.closest_to(se).position:
                    ...
                    if distance is None:
                        ...
                        self.combinedActions.append(se.move(se.position.towards(threats.closest_to(se).position, -1)))
                        # print('- Microing Colossus! -')
        ...
        for vr in self.units(VOIDRAY):
            ...
            if vr.shield_percentage < 0.1:
                threats = self.known_enemy_units.filter(
                    lambda unit: unit.type_id not in self.units_to_ignore
                ).closer_than(vr.air_range + vr.radius, vr.position)
                if threats.exists and vr.position != threats.closest_to(vr).position:
                    ...
                    if distance is None:
                        ...
                        self.combinedActions.append(vr.move(vr.position.towards(threats.closest_to(vr).position, -2)))
            ...
            # Prioritize targets with a specific armor type
                ...
                threats = self.known_enemy_units.filter(
                    lambda unit: unit.type_id not in self.units_to_ignore
                ).closer_than(vr.air_range + vr.radius + 2, vr.position)
                # print(threats)
                if threats.exists:
                    for threat in threats:
    ...
    async def two_base_colossus_buildings(self):
        if self.units(PYLON).ready.exists:
            ...
            if (
                (len(self.units(GATEWAY)) + len(self.units(WARPGATE))) < (self.time / 60)
                and (len(self.units(GATEWAY)) + len(self.units(WARPGATE))) < self.MAX_GATES
                and self.units(ROBOTICSFACILITY).ready.exists
            ):
    ...
    async def two_base_colossus_upgrade(self):
        if (
            self.units(ROBOTICSBAY).ready.exists
    ...
    async def two_base_colossus_offensive_force(self):
        ...
        for gw in self.units(GATEWAY).ready.noqueue:
            ...
            if (
                ...
                and not self.units(ROBOTICSBAY).ready.exists
            ):
                break
    ...
    async def two_base_colossus_unit_control(self):
        ...
        # attack_enemy_start
            ...
            if self.time > self.do_something_after:
                all_enemy_base = self.known_enemy_structures.filter(
                    lambda unit: unit.type_id not in self.units_to_ignore
                )
        ...
        if (
            self.first_attack
            and not self.known_enemy_structures.exists
    ...
    async def two_base_colossus_upgrade_lategame(self):
        ...
        if self.units(FORGE).ready.exists:
            ...
            if forge.noqueue and self.can_afford(
                FORGERESEARCH_PROTOSSGROUNDWEAPONSLEVEL1
        ...
        if self.units(TWILIGHTCOUNCIL).ready.exists and self.can_afford(RESEARCH_CHARGE) and not self.charge_started:
            twi = self.units(TWILIGHTCOUNCIL).ready.first
    ...
    async def immortal_adept_buildings(self):
        if self.units(PYLON).ready.exists:
            ...
            if (
                ...
                and not self.units(ROBOTICSFACILITY).ready.noqueue
                and self.MAX_GATES <= 6
            ):
                self.MAX_GATES = 7

            if (self.already_pending(ROBOTICSFACILITY) or self.units(ROBOTICSFACILITY).ready) and (
                len(self.units(GATEWAY)) + len(self.units(WARPGATE))
            ) < self.MAX_GATES:
    ...
    async def build_proxy_pylon_four_gate(self):
        if (
            (len(self.units(GATEWAY)) + len(self.units(WARPGATE))) >= 3
        ...
        if self.units(PYLON).exists:
            if self.units(NEXUS).exists:
                pylon = self.units(PYLON).ready.closest_to(
                    self.units(NEXUS).random.position.towards(self.game_info.map_center, random.randrange(1, 8))
                )
                nexus = self.units(NEXUS).random
            else:
                pylon = self.units(PYLON).ready.random
            ...
            if (
                len(self.units(SHIELDBATTERY)) >= 1
                and len(self.units(PHOTONCANNON)) >= 2
                and self.units(DARKSHRINE).exists
            ):
                self.MAX_GATES = 3
            else:
                self.MAX_GATES = 2
            ...
            if (len(self.units(GATEWAY)) + len(self.units(WARPGATE))) < self.MAX_GATES:
                if self.can_afford(GATEWAY):
                    await self.build(
                        GATEWAY,
                        near=self.units(PYLON).ready.random,
                        max_distance=10,
                        random_alternative=False,
                        placement_step=5,
                    )
            ...
            if (
                not self.units(TWILIGHTCOUNCIL).exists
            ...
            if (
                not self.units(DARKSHRINE).exists
                and not self.already_pending(DARKSHRINE)
            ):
                if self.can_afford(DARKSHRINE) and self.units(TWILIGHTCOUNCIL).ready.exists:
                    await self.build(DARKSHRINE, near=self.units(PYLON).ready.closest_to(
                    self.units(NEXUS).random.position.towards(self.game_info.map_center, random.randrange(-7, 2))))
            ...
            if (
                ...
                and self.units(TWILIGHTCOUNCIL).ready.exists
    ...
    async def one_base_dt_offensive_force(self):
        ...
        for gw in self.units(GATEWAY).ready.noqueue:
            if (
                ...
                and not self.already_pending(TWILIGHTCOUNCIL)
                and not self.units(TWILIGHTCOUNCIL).ready.exists
                and self.minerals < 150
            ):
                break
            elif (
                self.units(TWILIGHTCOUNCIL).ready.exists
                and not self.already_pending(DARKSHRINE)
                and not self.units(DARKSHRINE).ready.exists
                and self.minerals < 150
            ):
                break
            elif (
                self.units(GATEWAY).ready.exists
                and self.units(DARKSHRINE).ready.exists
                ...
                if self.first_attack:
                    for dt in self.units(DARKTEMPLAR).idle:
                        if self.known_enemy_units.of_type(UnitTypeId.SPORECRAWLER):
                            ...
                            self.combinedActions.append(
                                dt.attack(
                                    self.known_enemy_units.of_type(
    ...
    async def one_base_vr_buildings(self):
        ...
        if self.units(PYLON).exists:
            if self.units(NEXUS).exists:
                ...
                pylon = self.units(PYLON).ready.random
            if (len(self.units(GATEWAY)) + len(self.units(WARPGATE))) < self.MAX_GATES and len(self.units(STARGATE)) > 0:
                if self.can_afford(GATEWAY):
                    await self.build(
                        GATEWAY,
                        near=self.units(PYLON).ready.random,
                ...
                and len(self.units(SHIELDBATTERY)) < 2
                ...
                and self.can_afford(STARGATE)
                and not self.already_pending(STARGATE)
                and len(self.units(STARGATE).ready) < 2
    ...
    async def one_base_vr_offensive_force(self):
        ...
        for gw in self.units(GATEWAY).ready.noqueue:
            if (
                ...
                await self.do(gw.train(STALKER))
            elif self.units(GATEWAY).ready.exists and self.can_afford(ZEALOT) and self.supply_left > 1:
                await self.do(gw.train(ZEALOT))
            elif self.units(GATEWAY).ready.exists and self.minerals > 250 and self.supply_left > 1:
                await self.do(gw.train(ZEALOT))
    ...
    async def vr_unit_control(self):
        ...
        if len(self.units(ORACLE)) >= 1 and not self.harass_started:
            ...
            self.combinedActions.append(or1.move(Point2((safe_spot1, safe_spot2))))
            self.combinedActions.append(or1.move(save_target_main, queue=True))
            ...
            if self.time > self.do_something_after_trap1:
                ...
                if or1.shield_percentage > 0.5 and or1.energy_percentage > 0.25:
                    ...
                    await self.do(or1(BEHAVIOR_PULSARBEAMON))
                    if self.known_enemy_units.of_type({UnitTypeId.DRONE, UnitTypeId.PROBE, UnitTypeId.SCV}):
                        # print('Attacking Drone')
                        self.combinedActions.append(
                            or1.attack(
                                self.known_enemy_units.of_type(
                        ...
                        # print('Attacking Else')
                        self.combinedActions.append(or1.attack(attack_target_main))


                    # await self.do(or1(BUILD_STASISTRAP, attack_target_main))
                    ...
                    await self.do(or1(BEHAVIOR_PULSARBEAMOFF))
                    self.combinedActions.append(or1.move(save_target_main))
    ...
    async def destroy_lifted_buildings(self):
        ...
        if self.units(PYLON).ready.exists:
            ...
            if (
                ...
                and self.can_afford(STARGATE)
                and not self.already_pending(STARGATE)
                and len(self.units(STARGATE).ready) < 3
            ):
        ...
        for wg in self.units(WARPGATE).ready:
            abilities = await self.get_available_abilities(wg)
            if WARPGATETRAIN_ZEALOT in abilities:
                if self.units(WARPGATE).ready.exists and self.minerals > 850 and self.supply_left > 1:
                    await self.do(wg.warp_in(ZEALOT, self.units(PYLON).ready.random))
